...
def ingest_document(
    ...
    # Auto-promote lab/manual evidence to biomarker nodes (no edges — edges are QA-only)
    promoted_nodes = []
    if any(t in tags for t in ("lab", "manual", "inbody")):
        promoted_nodes = _promote_to_graph(user_id, inserted, document_date)

    return {
        "raw_id":               raw_id,
        "storage_path":         storage_path,
        "evidence_count":       len(inserted),
        "graph_nodes_created":  len(promoted_nodes),
...
def _promote_to_graph(user_id: str, evidence_rows: list[dict], document_date: str) -> list[dict]:
    """
    Auto-create biomarker snapshot nodes for lab/manual/inbody evidence.
    One node per distinct data_type on the document date.
    No edges are created — edges are QA-only (built during analysis).

    Node pattern: stable snapshot
        label:      "WBC 6.2 (2026-04-18)"
        properties: { value, unit, recorded_at, evidence_id, ref_range, flag }
    """
    created = []
    seen: set[str] = set()
    ...
    for row in evidence_rows:
        ...
        if value is not None:
            ...
            label = f"{data_type} {val_str}{(' ' + unit) if unit else ''} ({document_date})"
        elif value_text:
            label = f"{data_type} {value_text} ({document_date})"
        else:
            continue  # no value to anchor the node

        props: dict = {
            "recorded_at": document_date,
            "evidence_id": row.get("id"),
        }
        if value is not None:
            props["value"] = float(value)
        if value_text:
            props["value_text"] = value_text
        if unit:
            props["unit"] = unit
        if meta.get("ref_range"):
            props["ref_range"] = meta["ref_range"]
        if meta.get("flag"):
            props["flag"] = meta["flag"]

        try:
            node = graph_dal.upsert_entity(
                user_id=user_id,
            ...
            pass  # non-fatal — graph promote never blocks ingestion
